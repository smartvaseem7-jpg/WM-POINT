...
const CACHE_NAME = "superfriends-shell-v2";
const APP_SHELL = [
  "./",
  "./index.html",
  "./manifest.json",
...
self.addEventListener("fetch", (event) => {
  ...
  // Network-first: always try to get the freshest file first (so updates you
  // push show up immediately), only falling back to the cached copy when
  // there's no network at all (offline support).
  event.respondWith(
    fetch(event.request)
      .then((response) => {
